...
===================================''';
  ...
  Future<SupabaseOperationResult<void>> deleteNote(String noteId) async {
    try {
      ...
      // Get the note to find its folder before deleting
      final noteResult = await getNoteById(noteId);
      if (noteResult.success && noteResult.data != null) {
        final folderId = noteResult.data!.folderId;
        ...
        // Note not found, still try to delete in case of data inconsistency
        await _supabase
            .from('notes')
            .delete()
            .eq('id', noteId)
  ...
  Stream<List<NoteModel>> streamNotes(bool isDone) {
    if (!isAuthenticated) {
      return Stream.error('Please log in to access notes');
    }
...
{{success_criteria}}''',
  ...
  NoteModel _noteFromSupabaseMap(Map<String, dynamic> data) {
    return NoteModel(
      ...
      reminder:
          data['reminder'] != null
              ? ReminderModel.fromMap(data['reminder'] as Map<String, dynamic>)
              : null,
  ...
  /// Get daily note for a specific date (stub)
  ...
  /// Get weekly note for a specific date (stub)
  ...
  /// Get monthly note for a specific date (stub)
  ...
  /// Get daily note preferences (stub)
  Future<SupabaseOperationResult<Map<String, dynamic>>> getDailyNotePreferences(
    String userId,
  ) async {
    // Stub: return empty preferences
    return SupabaseOperationResult.success(<String, dynamic>{});
  }

  /// Save daily note preferences (stub)
